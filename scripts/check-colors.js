...
const theme = yaml.load(yamlFile);

// Extract base colors from the yaml structure
// In the YAML they are defined as anchors in dracula.base
// Since we're using js-yaml to load it, we need to find where they end up.
// Actually, since they are anchors, they might not be easily accessible
// if not referenced in the final object.
// Let's re-parse to specifically look for the base color values.
...
primaries.forEach(name => {
    ...
    console.log(`${name.padEnd(10)}: ${hex} | Lum: ${lum.toFixed(3)} | Contrast: ${contrast.toFixed(2)}`);
});
...
luminances.forEach(c => {
    const diff = Math.abs(c.lum - avgLum);
    if (diff > TOLERANCE) {
        console.error(`Error: Color ${c.name} luminance (${c.lum.toFixed(3)}) deviates too much from average (${avgLum.toFixed(3)})!`);
        hasError = true;
    }
});
